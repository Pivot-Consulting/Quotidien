import type { AppState, CalendarEvent, Exercise, Habit, Id, Note, Priority, Recurrence, Routine, Screen, Task, TrackingTab, PlanTab, WeekDay, WorkoutSession } from './types.js';
...
if (!appRoot) throw new Error('Conteneur #app introuvable');
...
let trackingTab: TrackingTab = 'habits';
let taskFilter: 'open' | 'today' | 'done' | 'all' = 'open';
let taskView: 'list' | 'matrix' = 'list';
let agendaView: 'month' | 'week' = 'month';
let noteFilter: 'active' | 'pinned' | 'archived' = 'active';
let noteSearch = '';
let selectedDate = today();
let calendarMonth = new Date(parseDate(today()).getFullYear(), parseDate(today()).getMonth(), 1);
let modalSubmit: ((form: HTMLFormElement) => Promise<void> | void) | null = null;
let unlocked = false;
let focusTaskId: Id | null = null;
let focusSeconds = 25 * 60;
let focusElapsed = 0;
let focusTimer: number | null = null;
let intervalTimer: number | null = null;
let deferredInstall: InstallPromptEvent | null = null;
let intervalState: { phase: 'effort' | 'rest'; remaining: number; round: number; rounds: number; effort: number; rest: number } | null = null;

interface InstallPromptEvent extends Event { prompt: () => Promise<void>; userChoice: Promise<{ outcome: 'accepted' | 'dismissed' }>; }

const WEEK_LABELS = ['D', 'L', 'M', 'M', 'J', 'V', 'S'];
const PRIORITY_LABEL: Record<Priority, string> = { normal: 'Normal', important: '⭐ Important', urgent: '🔥 Urgent' };
const RECURRENCE_LABEL: Record<Recurrence, string> = { '': 'Jamais', daily: 'Chaque jour', weekly: 'Chaque semaine', monthly: 'Chaque mois' };
const PRESET_PROGRAMS: Array<{ name: string; type: string; duration: number; exercises: Exercise[] }> = [
...
window.addEventListener('beforeinstallprompt', event => { event.preventDefault(); deferredInstall = event as InstallPromptEvent; });
setTimeout(handleInitialUrl, 0);
setInterval(checkReminders, 30_000);
document.addEventListener('visibilitychange', () => { if (!document.hidden) { selectedDate = today(); checkReminders(); render(); } });
...
function render(): void {
  if (state.preferences.pinHash && !unlocked) { renderLock(); return; }
  root.innerHTML = `
    <header class="topbar">
...
function navButton(value: Screen, icon: string, label: string): string {
...
function renderToday(): string {
  ...
  const manual = manualIds.map(id => allOpen.find(task => task.id === id)).filter((task): task is Task => Boolean(task));
...
function stat(value: string | number, label: string): string { return `<div class="stat"><strong>${value}</strong><span>${label}</span></div>`; }
...
function renderTasks(): string {
  ...
  if (taskFilter === 'today') tasks = tasks.filter(task => !task.completed && task.dueDate && task.dueDate <= today());
  if (taskFilter === 'done') tasks = tasks.filter(task => task.completed);
...
function filterButton(value: typeof taskFilter, label: string): string { return `<button class="chip ${taskFilter === value ? 'active' : ''}" data-task-filter="${value}">${label}</button>`; }
function recurrenceOptions(selected: Recurrence = ''): string { return (Object.entries(RECURRENCE_LABEL) as Array<[Recurrence,string]>).map(([value,label]) => `<option value="${value}" ${selected === value ? 'selected' : ''}>${label}</option>`).join(''); }

function taskSort(a: Task, b: Task): number {
  const score = (task: Task) => (task.urgent ? 4 : 0) + (task.important ? 2 : 0) + (task.dueDate && task.dueDate <= today() ? 1 : 0);
...
function renderTaskMatrix(tasks: Task[]): string {
  const groups = [
...
function eventsOn(date: string): CalendarEvent[] { return active(state.events).filter(event => occursOn(event.date, event.recurrence, date)).sort((a,b) => (a.time ?? '').localeCompare(b.time ?? '')); }
function upcomingEvents(days: number): Array<{ event: CalendarEvent; date: string }> { const result: Array<{event:CalendarEvent;date:string}> = []; for (let i=0;i<=days;i+=1) { const date = shiftDate(today(), i); eventsOn(date).forEach(event => result.push({ event, date })); } return result.slice(0, 20); }
function renderEventRow(event: CalendarEvent, withDelete = true): string {
...
function renderNotes(): string {
  ...
  if (noteFilter === 'active') notes = notes.filter(note => !note.archived);
  if (noteFilter === 'pinned') notes = notes.filter(note => note.pinned && !note.archived);
  if (noteFilter === 'archived') notes = notes.filter(note => note.archived);
  const query = noteSearch.trim().toLowerCase();
  if (query) notes = notes.filter(note => `${note.title} ${note.body} ${note.tags.join(' ')}`.toLowerCase().includes(query));
...
function noteFilterButton(value: typeof noteFilter, label: string): string { return `<button class="chip ${noteFilter === value ? 'active' : ''}" data-note-filter="${value}">${label}</button>`; }
...
function renderHabitToday(habit: Habit): string { const value = habit.completions[today()] ?? 0; const done = value >= habit.target; return `<div class="habit-row"><button class="habit-check ${done ? 'done' : ''}" data-action="toggle-habit" data-id="${habit.id}">${done ? '✓' : ''}</button><div><strong>${escapeHtml(habit.name)}</strong><span>${habit.target > 1 ? `${value}/${habit.target} ${escapeHtml(habit.unit)}` : `Série ${habitStreak(habit)} j`}</span></div></div>`; }
function renderHabitFull(habit: Habit): string {
  const monday = mondayOf(today()); const values = Array.from({length:7}, (_, i) => habit.completions[shiftDate(monday, i)] ?? 0);
...
function habitStreak(habit: Habit): number { let date = today(); let streak = 0; if ((habit.completions[date] ?? 0) < habit.target) date = shiftDate(date,-1); while ((habit.completions[date] ?? 0) >= habit.target) { streak += 1; date = shiftDate(date,-1); } return streak; }
...
function renderSport(): string {
  const sessions = active(state.sessions).sort((a,b) => b.date.localeCompare(a.date));
  const weekStart = mondayOf(today()); const week = sessions.filter(item => item.date >= weekStart);
...
function renderExerciseProgress(sessions: WorkoutSession[]): string {
  const names = [...new Set(sessions.flatMap(session => session.exercises.map(exercise => exercise.name)))];
  if (!names.length) return empty('Enregistre une séance avec des exercices pour suivre les charges.');
...
function sparkline(values: number[], unit: string): string { if (!values.length) return empty('Pas encore de données.'); const min=Math.min(...values),max=Math.max(...values),range=max-min||1; const pts=values.map((v,i)=>`${values.length===1?50:i/(values.length-1)*100},${38-(v-min)/range*30}`).join(' '); return `<svg class="spark" viewBox="0 0 100 45"><polyline points="${pts}" fill="none" stroke="currentColor" stroke-width="2"/><text x="98" y="12" text-anchor="end">${values.at(-1)} ${unit}</text></svg>`; }
function barChart(values: number[]): string { if(!values.length) return empty('Pas encore de données.'); const max=Math.max(...values,1); return `<div class="bars">${values.map(value=>`<i style="height:${Math.max(5,value/max*100)}%"><span>${value}</span></i>`).join('')}</div>`; }
function empty(message: string): string { return `<p class="empty">${escapeHtml(message)}</p>`; }

function bindGlobalEvents(): void {
  root.querySelectorAll<HTMLElement>('[data-screen]').forEach(element => element.addEventListener('click', () => { screen = element.dataset.screen as Screen; if (element.dataset.planTab) planTab = element.dataset.planTab as PlanTab; render(); }));
  root.querySelectorAll<HTMLElement>('[data-plan-tab]').forEach(element => element.addEventListener('click', () => { planTab = element.dataset.planTab as PlanTab; render(); }));
  root.querySelectorAll<HTMLElement>('[data-tracking-tab]').forEach(element => element.addEventListener('click', () => { trackingTab = element.dataset.trackingTab as TrackingTab; render(); }));
  root.querySelectorAll<HTMLElement>('[data-task-filter]').forEach(element => element.addEventListener('click', () => { taskFilter = element.dataset.taskFilter as typeof taskFilter; render(); }));
  root.querySelectorAll<HTMLElement>('[data-task-view]').forEach(element => element.addEventListener('click', () => { taskView = element.dataset.taskView as typeof taskView; render(); }));
  root.querySelectorAll<HTMLElement>('[data-agenda-view]').forEach(element => element.addEventListener('click', () => { agendaView = element.dataset.agendaView as typeof agendaView; render(); }));
  root.querySelectorAll<HTMLElement>('[data-note-filter]').forEach(element => element.addEventListener('click', () => { noteFilter = element.dataset.noteFilter as typeof noteFilter; render(); }));
  root.querySelectorAll<HTMLElement>('[data-project]').forEach(element => element.addEventListener('click', async () => store.setPreference('activeProject', element.dataset.project || null)));
  root.querySelectorAll<HTMLElement>('[data-date]').forEach(element => element.addEventListener('click', () => { selectedDate = element.dataset.date!; render(); }));
  root.querySelectorAll<HTMLElement>('[data-note-tag]').forEach(element => element.addEventListener('click', () => { noteSearch = element.dataset.noteTag ?? ''; render(); }));
  root.querySelectorAll<HTMLElement>('.wiki-link[data-note-title]').forEach(element => element.addEventListener('click', () => { noteSearch = element.dataset.noteTitle ?? ''; screen = 'notes'; render(); }));
  root.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(button => button.addEventListener('click', event => { event.preventDefault(); void handleAction(button.dataset.action!, button); }));
  root.querySelector<HTMLFormElement>('#task-form')?.addEventListener('submit', handleTaskForm);
  root.querySelector<HTMLFormElement>('#inbox-form')?.addEventListener('submit', handleInboxForm);
  root.querySelector<HTMLFormElement>('#session-form')?.addEventListener('submit', handleSessionForm);
  root.querySelector<HTMLFormElement>('#weight-form')?.addEventListener('submit', handleWeightForm);
  root.querySelector<HTMLFormElement>('#sleep-form')?.addEventListener('submit', handleSleepForm);
  root.querySelector<HTMLFormElement>('#meal-form')?.addEventListener('submit', handleMealForm);
  root.querySelector<HTMLFormElement>('#measurement-form')?.addEventListener('submit', handleMeasurementForm);
  root.querySelector<HTMLInputElement>('#note-search')?.addEventListener('input', event => { const value=(event.target as HTMLInputElement).value; noteSearch=value; render(); requestAnimationFrame(()=>{const input=root.querySelector<HTMLInputElement>('#note-search');if(input){input.focus();input.setSelectionRange(value.length,value.length);}}); });
  root.querySelector<HTMLFormElement>('#modal-form')?.addEventListener('submit', async event => { event.preventDefault(); if (modalSubmit) await modalSubmit(event.currentTarget as HTMLFormElement); closeModal(); });
}
...
async function handleAction(action: string, element: HTMLElement): Promise<void> {
  ...
  if (action === 'quick-capture') return openQuickCapture();
  if (action === 'quick-task') return openTaskModal();
  if (action === 'plan-day') return openPlanDay();
  if (action === 'weekly-review') return openWeeklyReview();
  if (action === 'toggle-task') return store.toggleTask(id);
  if (action === 'delete-task') return store.deleteTask(id);
  if (action === 'edit-task') return openTaskModal(state.tasks.find(item=>item.id===id));
  if (action === 'toggle-subtask') return store.toggleSubtask(id, element.dataset.subId!);
  if (action === 'focus') return startFocus(id);
  if (action === 'add-project') { const name=prompt('Nom de la nouvelle liste'); if(name) await store.addProject(name); return; }
  if (action === 'month-prev') { calendarMonth.setMonth(calendarMonth.getMonth()-1); return render(); }
  if (action === 'month-next') { calendarMonth.setMonth(calendarMonth.getMonth()+1); return render(); }
  if (action === 'week-prev') { selectedDate = shiftDate(selectedDate, -7); return render(); }
  if (action === 'week-next') { selectedDate = shiftDate(selectedDate, 7); return render(); }
  if (action === 'add-event') return openEventModal();
  if (action === 'edit-event') return openEventModal(state.events.find(item=>item.id===id));
  if (action === 'delete-event') return store.deleteEvent(id);
  if (action === 'share-event') { const event=state.events.find(item=>item.id===id); if(event) shareEvent(event); return; }
  if (action === 'export-ics') return exportIcs();
  if (action === 'import-ics') return importIcs();
  if (action === 'add-note') return openNoteModal();
  if (action === 'daily-journal') return openDailyJournal();
  if (action === 'note-graph') return openNoteGraph();
  if (action === 'export-markdown') return exportMarkdown();
  if (action === 'edit-note') return openNoteModal(state.notes.find(item=>item.id===id));
  if (action === 'pin-note') { const note=state.notes.find(item=>item.id===id); if(note) await store.updateNote(id,{pinned:!note.pinned}); return; }
  if (action === 'delete-note') return store.deleteNote(id);
  if (action === 'add-habit') return openHabitModal();
  if (action === 'edit-habit') return openHabitModal(state.habits.find(item=>item.id===id));
  if (action === 'toggle-habit') { const habit=state.habits.find(item=>item.id===id); if(habit) await store.setHabitValue(id,today(),(habit.completions[today()]??0)>=habit.target?0:habit.target); return; }
  if (action === 'habit-day') { const habit=state.habits.find(item=>item.id===id); const date=element.dataset.date!; if(habit) await store.setHabitValue(id,date,(habit.completions[date]??0)>=habit.target?0:habit.target); return; }
  if (action === 'delete-habit') return store.deleteHabit(id);
  if (action === 'add-routine') return openRoutineModal();
  if (action === 'edit-routine') return openRoutineModal(state.routines.find(item=>item.id===id));
  if (action === 'routine-step') return store.toggleRoutineStep(id,element.dataset.stepId!);
  if (action === 'delete-routine') return store.deleteRoutine(id);
  if (action === 'add-program') return openProgramModal();
  if (action === 'use-program') return useProgram(id);
  if (action === 'use-preset') return usePreset(Number(element.dataset.index));
  if (action === 'delete-program') return store.deleteProgram(id);
  if (action === 'delete-session') return store.deleteSession(id);
  if (action === 'interval-start') return toggleInterval();
  if (action === 'interval-reset') return resetInterval();
  if (action === 'water-minus') return store.changeWater(-1);
  if (action === 'water-plus') return store.changeWater(1);
  if (action === 'delete-meal') return store.deleteMeal(id);
  if (action === 'export-data') return exportData();
  if (action === 'import-data') return importData();
  if (action === 'enable-notifications') return enableNotifications();
  if (action === 'set-pin') return setPin();
  if (action === 'clear-pin') { await store.setPreference('pinHash',null); unlocked=true; return; }
  if (action === 'purge-trash') { await store.purgeDeleted(); toast('Corbeille nettoyée'); return; }
  if (action === 'restore-item') { await store.restore(element.dataset.type ?? '', id); toast('Élément restauré'); return; }
  if (action === 'install-app') return installApp();
}

async function handleTaskForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const form=event.currentTarget as HTMLFormElement; const data=new FormData(form); const urgent=data.get('urgent')==='on'; const important=data.get('important')==='on'; await store.addTask({ title:String(data.get('title')||''), dueDate:String(data.get('dueDate')||'')||null, dueTime:String(data.get('dueTime')||'')||null, reminderMinutes:data.get('reminderMinutes')===''?null:Number(data.get('reminderMinutes')), project:String(data.get('project')||'Personnel'), estimatedMinutes:Number(data.get('estimatedMinutes')||0), urgent, important, priority:urgent?'urgent':important?'important':'normal', recurrence:String(data.get('recurrence')) as Recurrence }); }
async function handleInboxForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const form=event.currentTarget as HTMLFormElement; const text=String(new FormData(form).get('text')||''); await store.addNote({title:text.slice(0,60),body:text,tags:['inbox']}); }
async function handleSessionForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const data=new FormData(event.currentTarget as HTMLFormElement); let exercises:Exercise[]=[]; try{exercises=JSON.parse(String(data.get('exercises')||'[]')) as Exercise[];}catch{} await store.addSession({type:String(data.get('type')),durationMinutes:Number(data.get('duration')),notes:String(data.get('notes')||''),exercises}); }
async function handleWeightForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const kg=Number(new FormData(event.currentTarget as HTMLFormElement).get('kg')); if(kg) await store.addWeight(kg); }
async function handleSleepForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const data=new FormData(event.currentTarget as HTMLFormElement); await store.addSleep(Number(data.get('hours')),Number(data.get('quality'))); }
async function handleMealForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const data=new FormData(event.currentTarget as HTMLFormElement); await store.addMeal(String(data.get('type')),String(data.get('description'))); }
async function handleMeasurementForm(event: SubmitEvent): Promise<void> { event.preventDefault(); const data=new FormData(event.currentTarget as HTMLFormElement); await store.addMeasurement(String(data.get('name')),Number(data.get('value')),String(data.get('unit'))); }

function openModal(title: string, body: string, submit: (form: HTMLFormElement)=>Promise<void>|void, submitLabel='Enregistrer'): void {
  const backdrop=root.querySelector<HTMLElement>('#modal-backdrop')!; backdrop.hidden=false;
  root.querySelector<HTMLElement>('#modal-title')!.textContent=title;
  const modalBody=root.querySelector<HTMLElement>('#modal-body')!; modalBody.innerHTML=body;
  const submitButton=root.querySelector<HTMLButtonElement>('#modal-form button[type="submit"]')!; submitButton.textContent=submitLabel; modalSubmit=submit;
  modalBody.querySelectorAll<HTMLElement>('[data-action]').forEach(button => button.addEventListener('click', event => { event.preventDefault(); void handleAction(button.dataset.action!, button); }));
}
function closeModal(): void { const backdrop=root.querySelector<HTMLElement>('#modal-backdrop'); if(backdrop) backdrop.hidden=true; modalSubmit=null; }
...
function openQuickCapture(): void {
  ...
  root.querySelectorAll<HTMLElement>('[data-capture]').forEach(button=>button.addEventListener('click',()=>{
    const type=button.dataset.capture;
    if(type==='task')openTaskModal();
    if(type==='event')openEventModal();
    if(type==='note')openNoteModal();
    if(type==='routine')openRoutineModal();
    if(type==='measurement')openMeasurementModal();
    if(type==='session'){closeModal();screen='tracking';trackingTab='sport';render();setTimeout(()=>root.querySelector('#session-form')?.scrollIntoView({behavior:'smooth'}),50);}
  }));
}
function openTaskModal(task?: Task): void {
  openModal(task ? 'Modifier la tâche' : 'Nouvelle tâche', `<div class="form-grid">
    ...
    const patch = {
      ...
      priority: urgent ? 'urgent' as Priority : important ? 'important' as Priority : 'normal' as Priority,
...
function openNoteModal(note?: Note): void {
  ...
  const template = root.querySelector<HTMLSelectElement>('#note-template');
  template?.addEventListener('change', () => {
    const body = root.querySelector<HTMLTextAreaElement>('#modal-form [name="body"]');
    if (!body) return;
...
function weekdayChecks(selected:WeekDay[]=[1,2,3,4,5,6,0]):string { return WEEK_LABELS.map((label,index)=>`<label class="day-check"><input type="checkbox" name="day" value="${index}" ${selected.includes(index as WeekDay)?'checked':''}><span>${label}</span></label>`).join(''); }
function openHabitModal(habit?:Habit): void { openModal(habit?'Modifier l’habitude':'Nouvelle habitude', `<div class="form-grid"><label class="span-2">Nom<input name="name" value="${escapeHtml(habit?.name??'')}" required></label><div class="span-2 day-picker">${weekdayChecks(habit?.days)}</div><label>Objectif hebdo<input name="weeklyGoal" type="number" min="1" max="7" value="${habit?.weeklyGoal??7}"></label><label>Cible par jour<input name="target" type="number" min="1" value="${habit?.target??1}"></label><label class="span-2">Unité<input name="unit" value="${escapeHtml(habit?.unit??'fois')}"></label></div>`,async form=>{const data=new FormData(form);const patch={name:String(data.get('name')),days:data.getAll('day').map(Number) as WeekDay[],weeklyGoal:Number(data.get('weeklyGoal')),target:Number(data.get('target')),unit:String(data.get('unit'))};habit?await store.updateHabit(habit.id,patch):await store.addHabit(patch);}); }
function openRoutineModal(routine?: Routine): void {
  openModal(routine ? 'Modifier la routine' : 'Nouvelle routine', `<div class="form-grid">
    ...
    if (routine) {
      ...
      if (index >= 0) next.routines[index] = { ...next.routines[index]!, name: String(data.get('name')), time: String(data.get('time')) || null, days: data.getAll('day').map(Number) as WeekDay[], steps, updatedAt: new Date().toISOString(), revision: next.routines[index]!.revision + 1, syncStatus: 'pending' };
      await store.replace(next);
    } else {
      await store.addRoutine({ name: String(data.get('name')), time: String(data.get('time')) || null, days: data.getAll('day').map(Number) as WeekDay[], steps });
    }
  });
}

function openMeasurementModal(): void {
...
function handleInitialUrl(): void {
  const params = new URLSearchParams(location.search);
  const requestedScreen = params.get('screen') as Screen | null;
  const capture = params.get('capture');
  const date = params.get('date');
  if (requestedScreen && ['today','plan','notes','tracking'].includes(requestedScreen)) screen = requestedScreen;
  if (date) selectedDate = date;
  if (requestedScreen === 'plan' && params.get('tab') === 'agenda') planTab = 'agenda';
  render();
  if (capture === 'task') setTimeout(openTaskModal, 50);
  if (capture === 'event') setTimeout(openEventModal, 50);
  if (capture === 'note') setTimeout(openNoteModal, 50);
  if (location.search) history.replaceState({}, '', location.pathname);
}

function openPlanDay(): void {
  ...
  const boxes = [...root.querySelectorAll<HTMLInputElement>('input[name="top3"]')];
  boxes.forEach(box => box.addEventListener('change', () => {
    const checked = boxes.filter(item => item.checked);
    if (checked.length > 3) { box.checked = false; toast('Maximum : trois priorités.'); }
  }));
}
...
function openNoteGraph(): void {
  ...
  const links:string[] = [];
  for (const source of notes) for (const target of notes) if (source.id !== target.id && source.body.toLowerCase().includes(`[[${target.title.toLowerCase()}`)) { const a=positions.get(source.id)!, b=positions.get(target.id)!; links.push(`<line x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"/>`); }
  ...
  root.querySelectorAll<SVGGElement>('[data-note-id]').forEach(node=>node.addEventListener('click',()=>{const note=state.notes.find(item=>item.id===node.dataset.noteId);if(note)openNoteModal(note);}));
}

function crc32(bytes: Uint8Array): number { let crc = 0xffffffff; for (const byte of bytes) { crc ^= byte; for (let index=0; index<8; index++) crc = (crc >>> 1) ^ (crc & 1 ? 0xedb88320 : 0); } return (crc ^ 0xffffffff) >>> 0; }
function zipMarkdown(files: Array<{name:string;content:string}>): Blob {
  const encoder=new TextEncoder(), parts:BlobPart[]=[], central:BlobPart[]=[]; let offset=0;
  const u16=(value:number)=>new Uint8Array([value&255,(value>>8)&255]); const u32=(value:number)=>new Uint8Array([value&255,(value>>8)&255,(value>>16)&255,(value>>>24)&255]);
  for(const file of files){const name=encoder.encode(file.name),data=encoder.encode(file.content),crc=crc32(data);const local=new Blob([new Uint8Array([0x50,0x4b,3,4]),u16(20),u16(0x0800),u16(0),u16(0),u16(0),u32(crc),u32(data.length),u32(data.length),u16(name.length),u16(0),name,data]);parts.push(local);central.push(new Blob([new Uint8Array([0x50,0x4b,1,2]),u16(20),u16(20),u16(0x0800),u16(0),u16(0),u16(0),u32(crc),u32(data.length),u32(data.length),u16(name.length),u16(0),u16(0),u16(0),u16(0),u32(0),u32(offset),name]));offset+=local.size;}
  const centralBlob=new Blob(central); return new Blob([...parts,centralBlob,new Uint8Array([0x50,0x4b,5,6]),u16(0),u16(0),u16(files.length),u16(files.length),u32(centralBlob.size),u32(offset),u16(0)],{type:'application/zip'});
}
function exportMarkdown(): void {
  const used=new Set<string>(); const files=active(state.notes).map(note=>{let base=note.title.replace(/[\\/:*?"<>|#[\]]/g,'').trim().slice(0,80)||'note';let name=base;let n=2;while(used.has(name))name=`${base} ${n++}`;used.add(name);const header=`---\ntags: [${note.tags.join(', ')}]\ncreated: ${note.createdAt.slice(0,10)}\n---\n\n`;return{name:`${name}.md`,content:header+note.body};});
  if (!files.length) { alert('Aucune note à exporter.'); return; }
  download(zipMarkdown(files),'quotidien-notes-obsidian.zip');
}

function usePreset(index: number): void {
  const preset=PRESET_PROGRAMS[index]; if(!preset)return;
  const form=root.querySelector<HTMLFormElement>('#session-form'); if(!form)return;
  (form.elements.namedItem('type') as HTMLSelectElement).value=preset.type;
  (form.elements.namedItem('duration') as HTMLInputElement).value=String(preset.duration);
  (form.elements.namedItem('notes') as HTMLTextAreaElement).value=`${preset.name}\n${preset.exercises.map(ex=>`${ex.name} ${ex.sets}×${ex.reps}${ex.weight?` @ ${ex.weight} kg`:''}`).join('\n')}`;
  (form.elements.namedItem('exercises') as HTMLInputElement).value=JSON.stringify(preset.exercises.map(ex=>({...ex,id:uid()})));
  form.scrollIntoView({behavior:'smooth'});
}

async function installApp(): Promise<void> {
  if (deferredInstall) { await deferredInstall.prompt(); await deferredInstall.userChoice; deferredInstall=null; return; }
...
function openSettings(): void {
  ...
  openModal('Réglages', `<div class="settings">
    ...
    const data=new FormData(form);
    const next=store.snapshot;
    next.preferences.theme=String(data.get('theme')) as AppState['preferences']['theme'];
...
function startFocus(id:Id):void {
  ...
  const toggle=root.querySelector<HTMLButtonElement>('#focus-toggle')!;
  toggle.addEventListener('click',()=>{
    if(focusTimer){clearInterval(focusTimer);focusTimer=null;toggle.textContent='Reprendre';}
    else{toggle.textContent='Pause';focusTimer=window.setInterval(()=>{focusSeconds=Math.max(0,focusSeconds-1);focusElapsed+=1;const display=root.querySelector<HTMLElement>('#focus-time');if(display)display.textContent=formatClock(focusSeconds);if(focusSeconds===0&&focusTimer){clearInterval(focusTimer);focusTimer=null;void store.addFocusSession(focusTaskId,taskTitle,Math.max(1,Math.round(focusElapsed/60)));void notify('Session terminée','Bravo, prends une courte pause.');}},1000);}
  });
  root.querySelector<HTMLButtonElement>('#focus-save')!.addEventListener('click',async()=>{if(focusTimer)clearInterval(focusTimer);focusTimer=null;if(focusElapsed>=30)await store.addFocusSession(focusTaskId,taskTitle,Math.max(1,Math.round(focusElapsed/60)));closeModal();});
  root.querySelector<HTMLButtonElement>('#focus-done')!.addEventListener('click',async()=>{if(focusTimer)clearInterval(focusTimer);focusTimer=null;if(focusElapsed>=30)await store.addFocusSession(focusTaskId,taskTitle,Math.max(1,Math.round(focusElapsed/60)));if(focusTaskId)await store.toggleTask(focusTaskId);closeModal();});
}
function toggleInterval():void { if(intervalTimer){clearInterval(intervalTimer);intervalTimer=null;render();return;}if(!intervalState){const effort=Number(root.querySelector<HTMLInputElement>('#interval-effort')?.value||30);const rest=Number(root.querySelector<HTMLInputElement>('#interval-rest')?.value||30);const rounds=Number(root.querySelector<HTMLInputElement>('#interval-rounds')?.value||8);intervalState={phase:'effort',remaining:effort,round:1,rounds,effort,rest};}intervalTimer=window.setInterval(()=>{if(!intervalState)return;intervalState.remaining-=1;if(intervalState.remaining<=0){if(intervalState.phase==='effort'&&intervalState.rest>0){intervalState.phase='rest';intervalState.remaining=intervalState.rest;beep(440);}else if(intervalState.round>=intervalState.rounds){resetInterval();notify('Entraînement terminé','Tous les intervalles sont terminés.');return;}else{intervalState.round+=1;intervalState.phase='effort';intervalState.remaining=intervalState.effort;beep(880);}}const time=root.querySelector<HTMLElement>('#interval-time');if(time)time.textContent=formatClock(intervalState.remaining);const phase=root.querySelector<HTMLElement>('#interval-phase');if(phase)phase.textContent=intervalState.phase==='effort'?'EFFORT':'REPOS';},1000);render(); }
function resetInterval():void { if(intervalTimer)clearInterval(intervalTimer);intervalTimer=null;intervalState=null;render(); }
function beep(frequency:number):void { try{const context=new AudioContext();const oscillator=context.createOscillator();oscillator.frequency.value=frequency;oscillator.connect(context.destination);oscillator.start();oscillator.stop(context.currentTime+.15);}catch{} }
function useProgram(id:Id):void {const program=state.programs.find(item=>item.id===id);if(!program)return;const form=root.querySelector<HTMLFormElement>('#session-form');if(!form)return;(form.elements.namedItem('type') as HTMLSelectElement).value='Musculation';(form.elements.namedItem('duration') as HTMLInputElement).value=(form.elements.namedItem('duration') as HTMLInputElement).value||'45';(form.elements.namedItem('notes') as HTMLTextAreaElement).value=`${program.name}\n${program.exercises.map(ex=>`${ex.name} ${ex.sets}×${ex.reps}${ex.weight?` @ ${ex.weight} kg`:''}`).join('\n')}`;(form.elements.namedItem('exercises') as HTMLInputElement).value=JSON.stringify(program.exercises);form.scrollIntoView({behavior:'smooth'}); }

function icsEscape(value:string):string{return value.replace(/\\/g,'\\\\').replace(/\n/g,'\\n').replace(/,/g,'\\,').replace(/;/g,'\\;');}
function eventIcs(event:CalendarEvent):string {const date=event.date.replaceAll('-','');const time=event.time?.replace(':','')??'';const start=event.time?`DTSTART:${date}T${time}00`:`DTSTART;VALUE=DATE:${date}`;const lines=['BEGIN:VEVENT',`UID:${event.id}@quotidien`, `DTSTAMP:${new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d{3}/,'')}`,start,`SUMMARY:${icsEscape(event.title)}`];if(event.durationMinutes&&event.time)lines.push(`DURATION:PT${event.durationMinutes}M`);if(event.location)lines.push(`LOCATION:${icsEscape(event.location)}`);if(event.notes)lines.push(`DESCRIPTION:${icsEscape(event.notes)}`);if(event.recurrence)lines.push(`RRULE:FREQ=${event.recurrence==='daily'?'DAILY':event.recurrence==='weekly'?'WEEKLY':'MONTHLY'}`);if(event.reminderMinutes!==null)lines.push('BEGIN:VALARM',`TRIGGER:-PT${event.reminderMinutes}M`,'ACTION:DISPLAY',`DESCRIPTION:${icsEscape(event.title)}`,'END:VALARM');lines.push('END:VEVENT');return lines.join('\r\n');}
function exportIcs():void {const content=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Quotidien V6//FR',...active(state.events).map(eventIcs),'END:VCALENDAR'].join('\r\n');download(new Blob([content],{type:'text/calendar'}),`quotidien-agenda-${today()}.ics`);}
function shareEvent(event:CalendarEvent):void {const content=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Quotidien V6//FR',eventIcs(event),'END:VCALENDAR'].join('\r\n');const file=new File([content],`${event.title.replace(/[^a-z0-9]/gi,'-')}.ics`,{type:'text/calendar'});if(navigator.canShare?.({files:[file]}))void navigator.share({files:[file],title:event.title});else download(file,file.name);}
function importIcs():void {const input=document.createElement('input');input.type='file';input.accept='.ics,text/calendar';input.onchange=async()=>{const file=input.files?.[0];if(!file)return;const text=await file.text();const blocks=text.replace(/\r?\n[ \t]/g,'').split('BEGIN:VEVENT').slice(1);const next=store.snapshot;for(const block of blocks){const get=(pattern:RegExp)=>block.match(pattern)?.[1]?.trim()??'';const title=get(/SUMMARY[^:]*:(.*)/);const dt=get(/DTSTART[^:]*:([0-9T]+)/);if(!title||dt.length<8)continue;const date=`${dt.slice(0,4)}-${dt.slice(4,6)}-${dt.slice(6,8)}`;const time=dt.includes('T')?`${dt.slice(9,11)}:${dt.slice(11,13)}`:null;next.events.push({...createImportedMeta(next.deviceId),kind:'event',title,date,time,durationMinutes:60,category:'other',location:get(/LOCATION[^:]*:(.*)/),notes:get(/DESCRIPTION[^:]*:(.*)/),recurrence:'',reminderMinutes:null,countdown:false});}await store.replace(next);toast(`${blocks.length} événement(s) importé(s)`);};input.click();}
function createImportedMeta(deviceId:string){const stamp=new Date().toISOString();return{id:uid(),createdAt:stamp,updatedAt:stamp,deletedAt:null,revision:1,deviceId,syncStatus:'local' as const};}

function exportData():void {const snapshot=store.snapshot;snapshot.preferences.lastExportAt=new Date().toISOString();download(new Blob([JSON.stringify(snapshot,null,2)],{type:'application/json'}),`quotidien-v6-${today()}.json`);void store.setPreference('lastExportAt',snapshot.preferences.lastExportAt);}
function importData():void {const input=document.createElement('input');input.type='file';input.accept='.json';input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{const parsed=JSON.parse(await file.text()) as AppState;if(parsed.schemaVersion!==6)throw new Error('Version incompatible');await store.replace(parsed);closeModal();toast('Sauvegarde restaurée');}catch(error){alert(`Import impossible : ${error instanceof Error?error.message:'fichier invalide'}`);}};input.click();}
async function setPin():Promise<void>{const code=prompt('Choisis un code de 4 à 8 chiffres');if(!code||!/^\d{4,8}$/.test(code)){alert('Le code doit contenir 4 à 8 chiffres.');return;}await store.setPreference('pinHash',await hash(code));unlocked=true;}
async function hash(value:string):Promise<string>{const buffer=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(`quotidien:${value}`));return [...new Uint8Array(buffer)].map(byte=>byte.toString(16).padStart(2,'0')).join('');}
function renderLock():void{root.innerHTML=`<main class="lock"><img src="./icon-192.png" alt=""><h1>Quotidien</h1><p>Entre ton code pour continuer.</p><form id="unlock-form"><input name="code" type="password" inputmode="numeric" maxlength="8" autofocus><button class="btn">Déverrouiller</button><span id="unlock-error"></span></form></main>`;root.querySelector<HTMLFormElement>('#unlock-form')!.addEventListener('submit',async event=>{event.preventDefault();const code=String(new FormData(event.currentTarget as HTMLFormElement).get('code'));if(await hash(code)===state.preferences.pinHash){unlocked=true;render();}else root.querySelector<HTMLElement>('#unlock-error')!.textContent='Code incorrect';});}

async function enableNotifications():Promise<void>{if(!('Notification'in window)){alert('Les notifications ne sont pas disponibles.');return;}const permission=await Notification.requestPermission();await store.setPreference('notificationsEnabled',permission==='granted');if(permission==='granted')await notify('Rappels activés','Quotidien pourra afficher tes rappels lorsque le système l’autorise.');}
async function notify(title:string,body:string,url='./'):Promise<void>{if(Notification.permission!=='granted')return;const registration=await navigator.serviceWorker?.ready; if(registration)await registration.showNotification(title,{body,icon:'./icon-192.png',badge:'./icon-192.png',data:{url}});else new Notification(title,{body});}
async function checkReminders():Promise<void>{
  await refreshDay();
  if(!state?.preferences.notificationsEnabled||Notification.permission!=='granted')return;
  const current=new Date(), date=today(), log=state.notificationLog;
  const due:Array<{key:string,title:string,body:string,url?:string}>=[];
  for(const event of eventsOn(date)){
    ...
    if(!log[key]&&current>=notifyAt&&current<=new Date(at.getTime()+15*60000))due.push({key,title:`📅 ${event.title}`,body:event.time?`${event.time}${event.location?` · ${event.location}`:''}`:'Aujourd’hui',url:`./?screen=plan&tab=agenda&date=${date}`});
  }
  for(const task of active(state.tasks).filter(item=>!item.completed&&item.dueDate===date&&item.reminderMinutes!==null)){
    ...
    if(!log[key]&&current>=notifyAt&&current<=new Date(at.getTime()+15*60000))due.push({key,title:`✓ ${task.title}`,body:'Tâche prévue aujourd’hui',url:'./?screen=plan'});
  }
  const minutes=current.getHours()*60+current.getMinutes();
  const toMinutes=(value:string)=>{const parts=value.split(':').map(Number);return (parts[0]??0)*60+(parts[1]??0);};
  ...
  const next=store.snapshot;
  for(const item of due){await notify(item.title,item.body,item.url);next.notificationLog[item.key]=new Date().toISOString();}
  await store.replace(next);
}
function registerServiceWorker():void{if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js').catch(console.error));}
function toast(message:string):void{const element=root.querySelector<HTMLElement>('#toast');if(!element)return;element.textContent=message;element.hidden=false;setTimeout(()=>{element.hidden=true;},2500);}
