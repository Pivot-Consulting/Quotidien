...
Implémentation additive sur les 20 OS et 48 modèles. Nouveaux modules métier `durable`, `connected`, `focus` ; UI séparée `drafts-ui`, `focus-ui`. Les écritures passent par un repository asynchrone et les boutons sont désactivés pendant les transactions.

Stockage : IndexedDB (`quotidien-local-v3`), snapshot de migration immuable, copie précédente et checkpoint dans une même transaction. Ancienne clé localStorage intacte ; modification par une ancienne version détectée. Une base absente après migration bloque le chargement plutôt que ressusciter silencieusement une ancienne copie. L’écran de récupération permet d’exporter les copies disponibles ; une réparation d’une base manquante reste une opération accompagnée, sans effacement automatique.

Reprise : dernier formulaire sauvegardé après saisie, réouverture depuis Réglages, export et suppression explicite. Une modification des données depuis le brouillon bloque sa réapplication. Le navigateur peut interrompre une transaction lors d’une fermeture brutale : cette protection ne garantit pas chaque dernière frappe.

Finances : `projectId` sur les mouvements et objectifs, graphe commun ; mode manuel conservé par défaut. Compte réservé à un objectif monétaire, progression calculée, hypothèses de projection explicites, alertes de budget projet et d’épargne. La duplication d’un objectif retire son affectation de compte pour éviter la double allocation.
...
Validation : `npm run verify`, 96 tests (dont les parcours des 48 modèles). Aperçu navigateur : Today sans débordement horizontal à 320, 390 et 844 px ; création de tâche, Focus, notes, pause et reprise après rechargement vérifiés. Ce contrôle responsive ne constitue pas un test de clavier natif iOS/Android ni d’installation PWA sur appareil physique.
