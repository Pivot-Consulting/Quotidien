import { LIFE_DOMAINS, LIFE_DOMAIN_BY_ID, LIFE_GROUP_LABELS } from './catalog.js';
import { addLifeEntry, deleteLifeEntry, exportLifeOsState, loadLifeOsState, replaceLifeOsState, toggleLifeDomain, updateLifeEntry } from './store.js';
const STATUS_LABELS = { idea: 'Idée', active: 'En cours', waiting: 'En attente', done: 'Terminé' };
const GROUPS = ['organiser', 'se-developper', 'prendre-soin', 'piloter'];
const escapeHtml = (value) => String(value ?? '').replace(/[&<>'"]/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[character] ?? character);
const today = () => { const date = new Date(); return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; };
let state = loadLifeOsState();
...
if (new URLSearchParams(location.search).get('lifeos') === '1')
    queueMicrotask(openHub);
function openHub() {
...
function render() {
    const content = overlay.querySelector('#life-os-content');
    if (!content)
        return;
...
function renderGroup(group, filteredEntries) {
...
function domainFrom(element) {
    const value = element.dataset.domain;
    return value && LIFE_DOMAIN_BY_ID.has(value) ? value : null;
}
async function handleClick(event) {
    if (event.target === overlay)
        return closeHub();
    const target = event.target.closest('[data-life-action]');
    if (!target)
        return;
    ...
    if (action === 'toggle-domain') {
        const domain = domainFrom(target);
        if (domain) {
            state = toggleLifeDomain(state, domain);
            render();
        }
        return;
    }
    if (action === 'starter') {
        const input = overlay.querySelector('#life-entry-title');
        if (input) {
    ...
    if (action === 'delete-entry') {
        ...
        if (id && confirm('Supprimer cet élément ?')) {
            state = deleteLifeEntry(state, id);
            render();
        }
        return;
    }
    if (action === 'export') {
        exportLifeOsState(state);
        return;
    }
    if (action === 'import')
        return importState();
...
async function handleSubmit(event) {
    ...
    if (form.id !== 'life-entry-form' || !activeDomain)
        return;
    event.preventDefault();
    const data = new FormData(form);
    const rawStatus = String(data.get('status') ?? 'active');
    const status = Object.hasOwn(STATUS_LABELS, rawStatus) ? rawStatus : 'active';
...
function handleInput(event) {
    ...
    if (target.id === 'life-os-search') {
        search = target.value;
        render();
        const input = overlay.querySelector('#life-os-search');
        input?.focus();
    ...
    if (target.dataset.lifeAction === 'entry-status') {
        ...
        if (id && Object.hasOwn(STATUS_LABELS, status)) {
            state = updateLifeEntry(state, id, { status });
            render();
        }
    }
}
function importState() {
    ...
    input.addEventListener('change', async () => {
        ...
        if (!file)
            return;
        try {
            state = replaceLifeOsState(JSON.parse(await file.text()));
            activeDomain = null;
            search = '';
            render();
        }
        catch {
            alert('Le fichier sélectionné ne contient pas une sauvegarde Life OS valide.');
        }
    });
    input.click();
}
