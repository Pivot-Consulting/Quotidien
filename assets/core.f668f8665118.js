...
(function (Q) {
    ...
    (function (Personal) {
        ...
        Personal.completed = (r) => r.done === true ||
            r.status === "Terminé" ||
        ...
        function blockers(s, ref) {
            return connections(s)
                ...
                .filter((h) => !Personal.completed(h.record));
        }
        ...
        function search(s, f = {}, today = Q.day()) {
            ...
            return Personal.all(s)
                ...
                return (!r.deleted &&
                    ...
                    (!f.status ||
                        (f.status === "done"
                            ? Personal.completed(r)
                            : f.status === "overdue"
                                ? !!r.due && String(r.due) < today && !Personal.completed(r)
                                : !Personal.completed(r))) &&
    ...
    (function (Planning) {
        function entries(s, includeDone = false) {
            ...
            for (const hit of Q.Personal.all(s)) {
                const r = hit.record;
                if (!Q.Personal.visible(r) || (!includeDone && Q.Personal.completed(r)))
                    continue;
