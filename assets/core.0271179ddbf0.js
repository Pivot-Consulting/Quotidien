...
(function (Q) {
    ...
    function empty() {
        ...
        }, Object.fromEntries(Q.collections.map((key) => [key, []])));
    }
    Q.empty = empty;
    ...
    function normalize(input) {
        ...
        Q.Routines.validate(next);
        Q.Vault.validate(next);
        Q.Automation.validate(next);
        Q.Evolution.validate(next);
        Q.Progression.validate(next);
        Q.Commands.validate(next);
        return next;
    }
    ...
    (function (OS) {
        ...
        function validate(record) {
            ...
            for (const f of m.fields) {
                ...
                if (f.type === "weights") {
                    Q.Progression.validateWeights(record);
                    continue;
                }
                if (f.type === "refs") {
                    if (!Array.isArray(v) ||
        ...
        function balances(state) {
            ...
            for (const r of OS.rows(state, "sharedExpense")) {
                ...
                for (const share of Q.Progression.shares(r))
...
(function (Q) {
    ...
    (function (Personal) {
        ...
        Personal.route = (ref) => "#record/" + ref.key + "/" + encodeURIComponent(ref.id);
        function parseRoute(hash) {
            const match = /^#record\/([^/]+)\/([^/]+)$/.exec(hash);
            if (!match || !Q.collections.includes(match[1]))
                return null;
            try {
                return { key: match[1], id: decodeURIComponent(match[2]) };
            }
            catch {
                return null;
            }
        }
        Personal.parseRoute = parseRoute;
        function statusOptions(s, ref) {
            const r = Personal.resolve(s, ref);
            if (!r || !Personal.visible(r))
                return [];
            if (ref.key === "tasks")
                return ["À faire", "Terminé"];
            if (ref.key === "os")
                return (Q.OS.getModel(r.kind)?.fields.find((f) => f.key === "status")?.options ||
                    []);
            return [];
        }
        Personal.statusOptions = statusOptions;
        function changeStatus(s, ref, value) {
            const r = Personal.resolve(s, ref);
            ...
            if (ref.key === "tasks")
                r.done = value === "Terminé";
            else
                r.status = value;
        }
        Personal.changeStatus = changeStatus;
        function domain(hit) {
            if (hit.key === "os")
        ...
        function validate(s) {
            ...
            if (explorer !== undefined &&
                (!obj(explorer) ||
                    ...
                    !obj(explorer.filter) ||
                    Object.entries(explorer.filter).some(([k, v]) => k === "favorite" ? typeof v !== "boolean" : typeof v !== "string")))
        ...
        function graph(s) {
            ...
            for (const h of Personal.all(s)) {
                ...
                if (h.key === "os" &&
                    ...
                    add(h, h.record.convertedFrom, h.id, "Registre d’origine");
                if (h.key === "finances") {
        ...
        function blockers(s, ref) {
            ...
            const children = ref.key === "tasks"
                ? Q.Evolution.children(s, ref.id).map((record) => ({
                    key: "tasks",
                    id: record.id,
                    record,
                }))
                : [];
            return [...linked, ...children].filter((h) => !Personal.completed(h.record, s));
        }
...
(function (Q) {
    ...
    (function (Intelligence) {
        const P = Q.Personal;
        const threshold = (s, key, fallback) => Number(s.settings.analysis?.[key] ||
            fallback);
        ...
        const deadlines = (s, today) => Q.Planning.entries(s)
            .filter((x) => x.deadline &&
            x.start <= Q.OS.addDays(today, threshold(s, "horizon", 7)))
            .map((x) => finding("deadline:" + x.id, x.start < today ? "Important" : "À surveiller", `${x.label} ${x.start < today ? "dépassée" : "à venir"} · ${P.title(x.hit.record)}`, `${x.label} le ${x.start}. ${x.start < today ? Q.OS.daysBetween(x.start, today) + " jour(s) de retard." : `Dans les ${threshold(s, "horizon", 7)} prochains jours.`}`, [x.hit], `Traiter ${x.label.toLowerCase()} · ${P.title(x.hit.record)}`, x.start));
        ...
        const projects = (s, today) => live(s, "project")
            ...
            if (!last ||
                Q.OS.daysBetween(last, today) < threshold(s, "stagnation", 30))
                return [];
            return [
        ...
        const workload = (s, today) => live(s, "planning")
            ...
            String(r.date) <= Q.OS.addDays(today, threshold(s, "horizon", 7)))
            .flatMap((r) => {
        ...
        class Repository {
            ...
            async open() {
                ...
                this.db = await new Promise((resolve, reject) => {
                    let blocked = false;
                    const r = this.factory.open(Durable.DB, 2);
                    r.onupgradeneeded = () => {
            ...
            async putFile(key, value) {
                if (!this.db)
                    throw new Error("Les pièces jointes nécessitent le stockage IndexedDB de ce navigateur.");
                ...
                return new Promise((resolve, reject) => {
                    ...
                    tx.oncomplete = () => resolve();
                    tx.onabort = () => reject(tx.error || new Error("Pièce jointe non enregistrée."));
                });
            }
            async getFile(key) {
                if (!this.db)
            ...
            async deleteFile(key) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    tx.oncomplete = () => resolve();
                    tx.onabort = () => reject(tx.error || new Error("Pièce jointe non retirée."));
                });
            }
            async clearFiles() {
                if (!this.db)
                ...
                return new Promise((resolve, reject) => {
                    ...
                    tx.oncomplete = () => resolve();
                    tx.onabort = () => reject(tx.error || new Error("Coffre non réinitialisé."));
                });
            }
            async load() {
                if (!this.db) {
                    const next = this.legacy.load();
                    this.expected = this.storage.getItem(Q.KEY);
                    return next;
                }
            ...
            async commit(state, checkpoint = false, files = []) {
                if (!this.db) {
                    if (files.length)
                ...
                await new Promise((resolve, reject) => {
                    const tx = this.db.transaction(files.length ? ["data", "files"] : ["data"], "readwrite"), st = tx.objectStore("data"), r = st.get("current");
                    let failure;
                    r.onsuccess = () => {
            ...
            async assertCurrent() {
                if (this.expected === undefined ||
                    ...
                    throw new Error("Les données ont changé dans un autre onglet. Recharge avant de recommencer.");
                if (this.db && (await this.read("legacy")) !== this.storage.getItem(Q.KEY))
                    throw new Error("Une ancienne version a modifié les données. Recharge avant de continuer.");
            }
            close() {
                this.db?.close();
            }
...
(function (Q) {
    ...
    (function (Routines) {
        ...
        function due(routine, date = Q.day()) {
            if (!Q.Personal.visible(routine))
                return false;
            const mode = String(routine.schedule || "Tous les jours");
            const weekday = new Date(date + "T12:00:00").getDay();
            if (mode === "Mensuel") {
                const d = new Date(date + "T12:00:00"), last = new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
                return d.getDate() === Math.min(Number(routine.monthDay || 1), last);
            }
            if (mode === "Intervalle")
                return (!!routine.anchor &&
                    date >= String(routine.anchor) &&
                    Q.OS.daysBetween(String(routine.anchor), date) %
                        Number(routine.everyDays) ===
                        0);
            if (mode === "Jours ouvrés")
        ...
        Routines.due = due;
        function run(s, routineId, date = Q.day()) {
            return (runs(s).find((x) => x.routineId === routineId && x.date === date) || null);
        }
        Routines.run = run;
        function start(s, routineId, id, date = Q.day()) {
            const routine = s.routines.find((x) => x.id === routineId && Q.Personal.visible(x));
            if (!routine)
            ...
            if (!Array.isArray(s.routineRuns))
                s.routineRuns = [];
            s.routineRuns.push(created);
            return created;
        }
        Routines.start = start;
        function toggle(s, routineId, index, date = Q.day()) {
        ...
        Routines.toggle = toggle;
        function complete(s, routineId, date = Q.day()) {
            const current = run(s, routineId, date);
            if (!current)
                throw new Error("Démarre la routine avant de la terminer.");
            if (current.steps.length && current.steps.some((x) => !x.done))
                throw new Error("Termine toutes les étapes ou laisse la routine ouverte.");
            current.completedAt || (current.completedAt = new Date().toISOString());
            return current;
        }
        Routines.complete = complete;
        function today(s, date = Q.day()) {
            return s.routines
                .filter((r) => due(r, date))
                .map((routine) => ({ routine, run: run(s, routine.id, date) }))
                .sort((a, b) => String(a.routine.time || "99:99").localeCompare(String(b.routine.time || "99:99")));
        }
        Routines.today = today;
        function history(s, routineId, limit = 30) {
            return runs(s)
        ...
        function installDefaults(s, makeId) {
            const templates = [
                {
                    templateKey: "morning",
                ...
                },
                {
                    ...
                    weekdays: "0",
                    steps: "Revue des projets\nPlanifier la semaine\nFinances et administration\nCourses et maison\nObjectifs et apprentissage",
                },
            ];
            let count = 0;
            for (const template of templates)
                if (!s.routines.some((r) => r.templateKey === template.templateKey && !r.deleted)) {
                    s.routines.push({
                        ...template,
                        id: makeId(),
        ...
        Routines.installDefaults = installDefaults;
        function validate(s) {
            for (const routine of s.routines) {
                ...
                if (schedule === "Intervalle" &&
                    (!Q.validDate(String(routine.anchor)) ||
                        !Number.isInteger(Number(routine.everyDays)) ||
                        Number(routine.everyDays) < 1 ||
                        Number(routine.everyDays) > 365))
                    throw new Error("Intervalle attendu entre 1 et 365 jours avec une date de départ.");
            ...
            if (s.routineRuns === undefined)
                return;
            if (!Array.isArray(s.routineRuns))
                throw new Error("Historique de routines invalide.");
            const tokens = new Set();
            for (const run of s.routineRuns) {
                if (!run ||
                    typeof run !== "object" ||
        ...
        Routines.validate = validate;
    })(Routines = Q.Routines || (Q.Routines = {}));
})(Q || (Q = {}));
...
(function (Q) {
    ...
    (function (Vault) {
        ...
        const fold = (v) => String(v || "")
            ...
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "");
        function expiry(r, today = Q.day()) {
            const date = String(r.expiry || r.date || "");
            if (!Q.validDate(date))
                return "none";
            if (date < today)
                return "expired";
            return date <= Q.OS.addDays(today, 30) ? "soon" : "later";
        }
        Vault.expiry = expiry;
        function search(s, query = "", status = "") {
            ...
            return s.documents
                ...
                (!term ||
                    fold([
                        ...
                        r.details,
                        r.fileName,
                        Q.Personal.tags(r).join(" "),
                    ].join(" ")).includes(term)))
                .sort((a, b) => String(a.expiry || "9999").localeCompare(String(b.expiry || "9999")) || Q.Personal.title(a).localeCompare(Q.Personal.title(b), "fr"));
        }
        Vault.search = search;
        function stats(s, today = Q.day()) {
            const live = s.documents.filter(Q.Personal.visible), attached = live.filter((x) => x.fileId);
            return {
                total: live.length,
        ...
        function validate(s) {
            for (const r of s.documents) {
                if (r.expiry !== undefined &&
                    r.expiry !== "" &&
                    (typeof r.expiry !== "string" || !Q.validDate(r.expiry)))
                    throw new Error("Date d’expiration documentaire invalide.");
                if (r.status !== undefined && !Vault.statuses.includes(String(r.status)))
...
(function (Q) {
    ...
    (function (Automation) {
        ...
        const logs = (s) => s.automationLogs || [];
        const notices = (s) => s.notifications || [];
        const lastActivity = (s, p) => [
            p.updatedAt,
            p.createdAt,
            ...s.tasks
                .filter((t) => t.osSourceId === p.id && !t.deleted)
                .flatMap((t) => [t.updatedAt, t.completedAt, t.createdAt]),
            ...Q.OS.rows(s, "milestone")
            ...
            .pop()
            ?.slice(0, 10) || "";
        function events(s, rule, today) {
            const horizon = Math.max(0, Number(rule.horizon || 7)), threshold = Math.max(0, Number(rule.threshold || 0));
            if (rule.trigger === "Tâche en retard")
                return s.tasks
                    .filter((t) => Q.Personal.visible(t) &&
                    !t.automationToken &&
                    !t.automationRuleId &&
                    !t.done &&
                    t.due &&
                    ...
                    title: `Tâche en retard · ${Q.Personal.title(t)}`,
            ...
            if (rule.trigger === "Dépense importante")
                return s.finances
                    ...
                    title: `Dépense à vérifier · ${Q.Personal.title(t)}`,
            ...
            if (rule.trigger === "Projet stagnant")
                return Q.OS.rows(s, "project")
                    .filter((p) => Q.Personal.visible(p) && !Q.OS.done(p))
                    .flatMap((p) => {
                    const last = lastActivity(s, p);
                    return last && Q.OS.daysBetween(last, today) > Math.max(1, horizon)
                        ? [
                            {
                                token: `project:${p.id}:${last}`,
                                title: `Projet stagnant · ${Q.Personal.title(p)}`,
            ...
            if (rule.trigger === "Voyage créé")
                return Q.OS.rows(s, "trip")
                    .filter(Q.Personal.visible)
                    .map((t) => ({
                    token: `trip:${t.id}`,
                    title: `Préparer le voyage · ${Q.Personal.title(t)}`,
                    message: `Une checklist peut être préparée pour ce voyage.`,
                    sourceKey: "os",
                    sourceId: t.id,
                    due: String(t.date || ""),
                }));
            if (rule.trigger === "Revue du dimanche" &&
        ...
        function run(s, today = Q.day(), makeId = () => Math.random().toString(36).slice(2)) {
            if (!Array.isArray(s.notifications))
                s.notifications = [];
            if (!Array.isArray(s.automationLogs))
                s.automationLogs = [];
            // Durable receipts are independent from the bounded display log.
            const used = new Set([
                ...(s.automationReceipts || []),
                ...logs(s).map((x) => x.token),
                ...s.tasks
                    .filter((t) => typeof t.automationToken === "string")
                    .map((t) => String(t.automationToken).replace(/:(Créer une tâche|Créer une checklist)$/, "")),
            ]);
            let count = 0;
            for (const rule of s.automations.filter((r) => Q.Personal.visible(r) && r.engineVersion === 1 && r.active === true))
                        ...
                        if (action === "Créer une tâche" ||
                            ...
                            if (!s.tasks.some((t) => t.automationToken === taskToken))
                                s.tasks.unshift({
                                    id: makeId(),
                                    title: event.title,
                                    due: event.due && Q.validDate(event.due) ? event.due : today,
                                    done: false,
                                    automationToken: taskToken,
                                    automationRuleId: rule.id,
                                    createdAt: new Date().toISOString(),
                                    personal: action === "Créer une checklist"
                                        ? {
            ...
            if (logs(s).length > 500)
                s.automationLogs = logs(s).slice(0, 500);
            return count;
        }
        Automation.run = run;
        function visible(s, today = Q.day()) {
            return notices(s)
        ...
        function snooze(s, id, days, today = Q.day()) {
            const n = notices(s).find((x) => x.id === id);
            if (!n)
                throw new Error("Notification introuvable.");
            n.snoozedUntil = Q.OS.addDays(today, days);
            n.read = true;
        }
        Automation.snooze = snooze;
        function read(s, id) {
            const n = notices(s).find((x) => x.id === id);
            if (n)
                n.read = true;
        ...
        function validate(s) {
            if (s.automationReceipts !== undefined &&
            ...
            for (const r of s.automations.filter((x) => x.engineVersion !== undefined)) {
                if (r.engineVersion !== 1 ||
                    typeof r.active !== "boolean" ||
                    !Automation.triggers.includes(String(r.trigger)) ||
                    !Array.isArray(r.actions) ||
                    !r.actions.length ||
                    r.actions.some((x) => !Automation.actions.includes(String(x))))
            ...
            if (s.notifications !== undefined &&
                (!Array.isArray(s.notifications) ||
                    s.notifications.some((n) => !n ||
                        typeof n.id !== "string" ||
                        typeof n.title !== "string" ||
                        typeof n.message !== "string" ||
                        typeof n.read !== "boolean" ||
                        !Number.isFinite(Number(n.priority)) ||
                        (n.snoozedUntil !== undefined && !Q.validDate(n.snoozedUntil)))))
                throw new Error("Notifications invalides.");
            if (s.automationLogs !== undefined &&
                (!Array.isArray(s.automationLogs) ||
                    s.automationLogs.some((l) => !l ||
                        typeof l.id !== "string" ||
...
(function (Q) {
    var FullBackup;
    (function (FullBackup) {
        FullBackup.MAX_BYTES = 100 * 1024 * 1024;
        FullBackup.MAX_ARCHIVE_BYTES = 160 * 1024 * 1024;
        function fileIds(state) {
            const ids = new Set();
            const add = (r) => {
        ...
        FullBackup.fileIds = fileIds;
        async function hash(bytes) {
            if (!globalThis.crypto?.subtle)
                throw new Error("La vérification d’intégrité nécessite HTTPS ou localhost.");
        ...
        async function create(state, repository) {
            const next = Q.normalize(state), files = [];
            let total = 0;
            for (const id of fileIds(next)) {
                ...
                total += blob.size;
                if (total > FullBackup.MAX_BYTES)
                    throw new Error("La sauvegarde complète est limitée à 100 Mo de fichiers. Télécharge séparément les pièces supplémentaires.");
            ...
            if (new Blob([raw]).size > FullBackup.MAX_ARCHIVE_BYTES)
                throw new Error("La sauvegarde complète dépasse 160 Mo.");
            return raw;
        }
        FullBackup.create = create;
        async function prepare(raw) {
            ...
            return { state: Q.normalize(state), files };
        }
        FullBackup.prepare = prepare;
    })(FullBackup = Q.FullBackup || (Q.FullBackup = {}));
})(Q || (Q = {}));
...
(function (Q) {
    var Evolution;
    (function (Evolution) {
        const live = (s, kind) => Q.OS.rows(s, kind).filter(Q.Personal.visible);
        const n = Q.OS.n;
        const field = (key, label, type = "text", extra = {}) => ({ key, label, type, ...extra });
        const number = (key, label, min = 0) => field(key, label, "number", { min, required: true });
        const date = (key, label) => field(key, label, "date", { required: true });
        const ref = (key, label, kind) => field(key, label, "ref", { ref: kind, required: true });
        const choice = (key, label, options) => field(key, label, "select", { options });
        function add(domain, id, label, fields, scheduled = false) {
            const model = { id, label, fields, scheduled };
            Q.OS.domains.find((d) => d.id === domain).models.push(model);
            Q.OS.models.push(model);
        }
        add("finance", "commitment", "Échéances financières", [
        ...
        add("security", "recovery", "Procédures de récupération", [
        ...
        add("nutrition", "shopping", "Courses", [
            field("item", "Aliment", "text", { required: true }),
        ...
        for (const kind of ["document", "equipment", "procedure"])
            Q.OS.getModel(kind).fields.push(field("vaultDocumentId", "Document du coffre", "vaultref"));
        Q.OS.getModel("application").fields.push(field("contactId", "Contact", "ref", { ref: "contact" }));
        Q.OS.getModel("course").fields.push(field("careerSkillId", "Compétence visée", "ref", { ref: "careerSkill" }));
        function children(s, id) {
            return s.tasks.filter((r) => Q.Personal.visible(r) && r.parentTaskId === id);
        }
        Evolution.children = children;
        function validate(s) {
            for (const key of Q.collections)
                        ...
                        for (const c of r.customFields) {
                            ...
                            if (!((c.type === "text" && typeof c.value === "string") ||
                                ...
                                (c.type === "date" &&
                                    ...
                                    Q.validDate(c.value))))
                                throw new Error("Valeur de champ personnalisé incompatible avec son type.");
                        }
                    }
                }
            ...
            const edge = (from, to) => dependencies.set(from, [...(dependencies.get(from) || []), to]);
            for (const t of s.tasks)
                if (t.parentTaskId)
                    edge(Q.Personal.token({ key: "tasks", id: String(t.parentTaskId) }), Q.Personal.token({ key: "tasks", id: t.id }));
            for (const c of Q.Personal.connections(s))
                if (!c.deleted && c.type === "depends")
                    edge(Q.Personal.token(c.from), Q.Personal.token(c.to));
            const visiting = new Set(), visited = new Set();
            const visit = (id) => {
                if (visiting.has(id))
            ...
            for (const r of s.os) {
                ...
                if (r.kind === "optionScore") {
                    const option = s.os.find((x) => x.id === r.optionId), criterion = s.os.find((x) => x.id === r.criterionId);
            ...
            for (const t of s.finances.filter((x) => !x.deleted && x.commitmentId)) {
                const c = s.os.find((x) => x.id === t.commitmentId && x.kind === "commitment");
                if (!c ||
                    paid.has(c.id) ||
            ...
            const config = s.settings.analysis;
            if (config)
                for (const [k, max] of [
                    ["horizon", 90],
                    ["stagnation", 365],
        ...
        Evolution.validate = validate;
        function paid(s, id) {
            return s.finances.some((t) => !t.deleted && t.commitmentId === id);
        }
        Evolution.paid = paid;
        function installments(s, accountId, title, total, count, first, makeId) {
            if (!title.trim() ||
                ...
                throw new Error("Échéancier invalide : compte, titre, montant signé, date et 1 à 60 mensualités requis.");
            const cents = Math.round(total * 100), part = Math.trunc(cents / count), group = makeId(), original = new Date(first + "T12:00:00");
            for (let i = 0; i < count; i++) {
                const date = new Date(original);
                date.setDate(1);
                date.setMonth(date.getMonth() + i);
                date.setDate(Math.min(original.getDate(), new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate()));
                s.os.push({
                    id: makeId(),
                    kind: "commitment",
        ...
        Evolution.installments = installments;
        function settle(s, id, makeId, today = Q.day()) {
            const c = live(s, "commitment").find((x) => x.id === id);
            if (!c || paid(s, id))
                throw new Error("Échéance absente ou déjà réglée.");
            s.finances.push({
                id: makeId(),
                label: c.title,
                type: "Autre",
        ...
        Evolution.settle = settle;
        function forecast(s, accountId, until) {
            if (!Q.validDate(until))
                throw new Error("Horizon invalide.");
            const actual = Q.Connected.balance(s, accountId);
            const commitments = live(s, "commitment").filter((c) => c.accountId === accountId && String(c.due) <= until && !paid(s, c.id));
            const expected = commitments.reduce((sum, c) => sum + n(c, "amount"), 0);
            return { actual, expected, projected: actual + expected, commitments };
        }
        Evolution.forecast = forecast;
        function decisionScores(s, decisionId) {
            const criteria = live(s, "criterion").filter((c) => c.decisionId === decisionId);
            return live(s, "option")
                .filter((o) => o.decisionId === decisionId)
                .map((option) => {
                const scores = criteria.map((c) => ({
                    criterion: c,
                    rating: live(s, "optionScore").find((x) => x.optionId === option.id && x.criterionId === c.id),
                }));
                ...
                return {
                    ...
                    complete,
                    score: complete
                        ? scores.reduce((a, x) => a + n(x.criterion, "weight") * n(x.rating, "score"), 0) / criteria.reduce((a, c) => a + n(c, "weight"), 0)
                        : null,
                };
            });
        }
        Evolution.decisionScores = decisionScores;
        const itemKey = (r) => JSON.stringify([Q.Personal.fold(String(r.item)).trim(), r.unit]);
        function shoppingNeeds(s, from, until) {
            if (!Q.validDate(from) || !Q.validDate(until) || until < from)
                throw new Error("Période de menus invalide.");
            const needs = new Map();
            for (const menu of live(s, "menu").filter((m) => !m.consumedAt && String(m.date) >= from && String(m.date) <= until)) {
                const ingredients = live(s, "ingredient").filter((i) => i.recipeId === menu.recipeId);
                if (!ingredients.length)
                    throw new Error("Ajoute les ingrédients structurés des recettes du menu.");
                for (const i of ingredients) {
                    const key = itemKey(i), current = needs.get(key) || {
                        item: String(i.item),
                        unit: String(i.unit),
                        quantity: 0,
                    };
            ...
            for (const [key, value] of needs)
                value.quantity = Math.max(0, value.quantity -
                    live(s, "stock")
                        .filter((r) => itemKey(r) === key)
                        .reduce((a, r) => a + n(r, "quantity"), 0) -
                    live(s, "shopping")
                        .filter((r) => itemKey(r) === key && !r.receivedAt && !Q.OS.done(r))
                        .reduce((a, r) => a + n(r, "quantity"), 0));
            return [...needs.values()].filter((r) => r.quantity > 0.000001);
        }
        Evolution.shoppingNeeds = shoppingNeeds;
        function makeShopping(s, from, until, makeId) {
        ...
        Evolution.makeShopping = makeShopping;
        function receive(s, id, makeId) {
            ...
            if (stock)
                stock.quantity = n(stock, "quantity") + n(r, "quantity");
            else
                s.os.push({
                    id: makeId(),
                    kind: "stock",
        ...
        Evolution.receive = receive;
        function consume(s, id) {
            const m = live(s, "menu").find((x) => x.id === id);
            if (!m || m.consumedAt)
                throw new Error("Menu absent ou déjà consommé.");
            const ingredients = live(s, "ingredient").filter((i) => i.recipeId === m.recipeId);
            if (!ingredients.length)
                throw new Error("Recette sans ingrédients structurés.");
            const needs = new Map();
            for (const i of ingredients)
                needs.set(itemKey(i), (needs.get(itemKey(i)) || 0) + n(i, "quantity") * n(m, "portions"));
            for (const [key, quantity] of needs)
                if (live(s, "stock")
                    ...
                    1e-9 <
                    quantity)
                    throw new Error("Stock insuffisant. Réceptionne les courses avant de consommer ce menu.");
            for (const [key, quantity] of needs) {
        ...
        Evolution.consume = consume;
        function convert(s, key, id) {
            ...
            if (existing) {
                existing.personal = { ...Q.Personal.meta(existing), archived: false };
            }
            else
                s.os.push({
                    id,
                    title: Q.Personal.title(source),
                    status: "En cours",
                    convertedFrom: key,
                    personal: { ...Q.Personal.meta(source), archived: false },
                    ...(source.assigneeId ? { assigneeId: source.assigneeId } : {}),
                    ...(source.customFields
                        ? { customFields: Q.clone(source.customFields) }
                        : {}),
                    ...(key === "assets"
                        ? {
                            kind: "equipment",
            ...
            source.conversionArchivedBefore = !!Q.Personal.meta(source).archived;
            source.personal = { ...Q.Personal.meta(source), archived: true };
            source.convertedTo = id;
            return { key: "os", id };
        }
        Evolution.convert = convert;
        function reverse(s, key, id) {
            const source = s[key].find((r) => r.id === id), target = s.os.find((r) => r.id === id && r.convertedFrom === key);
            if (!source?.convertedTo || !target)
                throw new Error("Conversion introuvable.");
            source.personal = {
                ...Q.Personal.meta(source),
                archived: !!source.conversionArchivedBefore,
            };
            delete source.convertedTo;
            target.personal = { ...Q.Personal.meta(target), archived: true };
        }
        Evolution.reverse = reverse;
        function lifeScores(s, today = Q.day()) {
            const since = Q.OS.addDays(today, -29);
            return [
                "Travail",
                "Relations",
                "Santé",
                ...
                const rows = live(s, "lifeRating").filter((r) => r.area === area && String(r.date) >= since && String(r.date) <= today);
                return {
                    area,
                    count: rows.length,
                    score: rows.length
                        ? Math.round((rows.reduce((a, r) => a + n(r, "score"), 0) / rows.length) * 10)
                        : null,
        ...
        Evolution.lifeScores = lifeScores;
        function monthlyReview(s, makeId) {
            ...
            s.routines.push({
                ...
                monthDay: 1,
                time: "18:00",
                steps: "Comparer réel et prévisionnel\nRevoir les projets et décisions\nÉvaluer les huit domaines de vie\nRelire le journal\nChoisir les priorités du mois",
            });
        }
        Evolution.monthlyReview = monthlyReview;
    })(Evolution = Q.Evolution || (Q.Evolution = {}));
})(Q || (Q = {}));
...
(function (Q) {
    var Commands;
    (function (Commands) {
        Commands.history = (s) => (s.commandHistory || []);
        Commands.examples = [
            "tâche Appeler Paul",
        ...
        Commands.registry = [
            ...
            },
            {
                ...
                parse: (s, m) => {
                    const matches = s.tasks.filter((r) => Q.Personal.visible(r) &&
                        !r.done &&
                        Q.Personal.fold(Q.Personal.title(r)) === Q.Personal.fold(m[1].trim()));
                    if (matches.length !== 1)
        ...
        function parse(s, text) {
            for (const handler of Commands.registry) {
            ...
            throw new Error("Commande non reconnue. Utilise tâche, note, chercher ou planifier, avec une date au format AAAA-MM-JJ.");
        }
        Commands.parse = parse;
        function check(i) {
            if (!["task", "note", "search", "plan"].includes(i.type) ||
                !i.text.trim() ||
                i.text.length > 2000 ||
                (i.date && !Q.validDate(i.date)))
                throw new Error("Commande ou date invalide.");
        }
        function apply(s, i, makeId) {
            check(i);
            if (i.type === "search")
                throw new Error("La recherche ne modifie pas les données.");
            if (i.source &&
                (Commands.history(s).some((h) => h.source === i.source && !h.undone) ||
                    s.tasks.some((t) => t.commandSource === i.source && (!t.commandUndone || !t.deleted))))
                throw new Error("Cette proposition a déjà été appliquée.");
            let before = null, after, ref;
            if (i.type === "plan") {
                const r = i.target && Q.Personal.resolve(s, i.target);
                if (!r || i.target.key !== "tasks" || JSON.stringify(r) !== i.expected)
                    throw new Error("La tâche a changé. Prévisualise à nouveau la commande.");
                before = Q.clone(r);
                after = { ...r, due: i.date };
                ref = i.target;
            }
            ...
            const candidate = Q.clone(s);
            candidate[ref.key] = [
                ...candidate[ref.key].filter((r) => r.id !== ref.id),
                after,
            ];
            candidate.commandHistory = [entry, ...Commands.history(candidate)].slice(0, 200);
            Object.assign(s, Q.normalize(candidate));
            return entry;
        }
        ...
        function content(r) {
            if (!r)
                return "";
            const copy = Q.clone(r);
            delete copy.updatedAt;
            delete copy.createdAt;
            return JSON.stringify(copy);
        }
        function undo(s, id) {
            const candidate = Q.clone(s), h = Commands.history(candidate).find((x) => x.id === id);
            if (!h || h.undone)
                throw new Error("Commande déjà annulée ou absente.");
            const r = Q.Personal.resolve(candidate, h.ref);
            if (content(r) !== content(h.after))
                throw new Error("Objet modifié depuis la commande : annulation refusée pour préserver tes changements.");
            candidate[h.ref.key] = candidate[h.ref.key].map((x) => x.id === h.ref.id
                ? h.before || {
                    ...x,
                    deleted: true,
        ...
        Commands.undo = undo;
        function validate(s) {
            if (s.commandHistory !== undefined &&
                (!Array.isArray(s.commandHistory) ||
                    s.commandHistory.length > 200 ||
                    Commands.history(s).some((h) => !h ||
                        typeof h.id !== "string" ||
                        typeof h.at !== "string" ||
                        typeof h.label !== "string" ||
                        !h.ref ||
                        !Q.collections.includes(h.ref.key) ||
                        !h.after ||
                        h.ref.id !== h.after.id ||
                        !Q.Personal.resolve(s, h.ref))))
                throw new Error("Journal des commandes invalide.");
            const agents = s.settings.agents;
            if (agents !== undefined &&
                (!Array.isArray(agents) ||
                    agents.some((a) => typeof a !== "string" || !Q.OS.domains.some((d) => d.id === a))))
                throw new Error("Permission d’agent invalide.");
        }
        Commands.validate = validate;
        function proposals(s, domain, today = Q.day()) {
            if (!(s.settings.agents || []).includes(domain))
                return [];
            // An agent only sees records from its explicitly granted domain.
            const scoped = Q.clone(s);
            for (const key of Q.collections)
                scoped[key] = scoped[key].filter((record) => Q.Personal.domain({ key, id: record.id, record }) === domain);
            scoped.connections = Q.Personal.connections(s).filter((c) => Q.Personal.resolve(scoped, c.from) && Q.Personal.resolve(scoped, c.to));
            scoped.activity = Q.Personal.revisions(s).filter((h) => Q.Personal.resolve(scoped, h.ref));
            return Q.Intelligence.analyze(scoped, today)
                .filter((f) => f.refs.length &&
                f.refs.every((ref) => {
                    const record = Q.Personal.resolve(s, ref);
                    return record && Q.Personal.domain({ ...ref, record }) === domain;
                }))
                .filter((f) => !Commands.history(s).some((h) => h.source === f.id && !h.undone) &&
                !s.tasks.some((t) => t.commandSource === f.id && (!t.commandUndone || !t.deleted)));
        }
        Commands.proposals = proposals;
        function accept(s, domain, findingId, makeId) {
            const f = proposals(s, domain).find((x) => x.id === findingId);
            if (!f)
                throw new Error("Proposition périmée ou permission retirée.");
            return apply(s, {
                type: "task",
                text: f.action,
                date: Q.day(),
    ...
    })(Commands = Q.Commands || (Q.Commands = {}));
})(Q || (Q = {}));
...
(function (Q) {
    ...
    (function (Progression) {
        const rows = (s, kind) => Q.OS.rows(s, kind).filter(Q.Personal.visible);
        const num = (key, label, min = 0, max = 1e6) => ({
            key,
            label,
            type: "number",
            min,
        ...
        function add(id, label, fields) {
            const model = { id, label, fields };
            Q.OS.domains.find((d) => d.id === "health").models.push(model);
            Q.OS.models.push(model);
        }
        ...
        Q.OS.getModel("sharedExpense").fields.push({
            key: "shareWeights",
            label: "Poids de répartition (facultatifs)",
            type: "weights",
        });
        Q.OS.getModel("sharedExpense").fields.find((f) => f.key === "participants").label = "Participants";
        function validateWeights(r) {
            if (r.shareWeights === undefined)
                return;
            const weights = r.shareWeights, people = r.participants;
            if (!Array.isArray(weights) ||
                ...
                weights.some((w) => !w ||
                    ...
                    w.weight <= 0 ||
                    w.weight > 1e6))
                throw new Error("Renseigne un poids positif pour chaque participant coché, et uniquement ces participants, ou laisse tous les poids vides pour des parts égales.");
        }
        Progression.validateWeights = validateWeights;
        function shares(r) {
            validateWeights(r);
            const people = (r.participants || []);
            if (!people.length)
                return [];
            const cents = Math.round(Q.OS.n(r, "cost") * 100);
            if (!Number.isSafeInteger(cents) || cents < 0)
                throw new Error("Montant partagé trop grand ou invalide.");
            const weights = people.map((id) => r.shareWeights?.find((w) => w.memberId === id)?.weight ?? 1), total = weights.reduce((a, b) => a + b, 0);
            ...
            if (remaining < 0 || remaining > parts.length)
                throw new Error("Précision monétaire insuffisante pour cette répartition.");
        ...
        Progression.shares = shares;
        function validate(s) {
            const seen = new Set();
            for (const r of s.os) {
                if (r.kind === "sharedExpense")
                    shares(r);
                if (r.kind === "trainingExercise")
                    for (const key of ["targetSets", "targetReps", "restSeconds"])
                        if (!Number.isInteger(Number(r[key])))
                            throw new Error("Les séries, répétitions et secondes attendent des entiers.");
                if (r.kind !== "exerciseSet")
                    continue;
                for (const key of ["setIndex", "repetitions", "seconds"])
                    if (!Number.isInteger(Number(r[key])))
                        throw new Error("Numéro de série, répétitions et durée : entiers requis.");
                const session = s.os.find((x) => x.id === r.sessionId), exercise = s.os.find((x) => x.id === r.exerciseId);
                if (session?.programId !== exercise?.programId)
                    throw new Error("La séance et l’exercice doivent appartenir au même programme.");
                if (r.deleted)
                    continue;
                const token = JSON.stringify([r.sessionId, r.exerciseId, r.setIndex]);
                if (seen.has(token))
                    throw new Error("Ce numéro de série existe déjà pour cet exercice dans cette séance.");
                seen.add(token);
            }
        }
        Progression.validate = validate;
        function performance(s, exerciseId) {
        ...
        Progression.performance = performance;
        const mean = (values) => values.length ? values.reduce((a, b) => a + b, 0) / values.length : null;
        function scores(s, today = Q.day()) {
            if (!Q.validDate(today))
                throw new Error("Date de bilan invalide.");
            const since = Q.OS.addDays(today, -29), week = Q.OS.addDays(today, -6);
            const recent = (r) => String(r.date) >= since && String(r.date) <= today;
            const component = (label, value, explanation, records, key = "os") => ({
                label,
                score: value === null ? null : Math.round(Q.OS.clamp(value)),
                explanation,
                refs: records.map((r) => ({ key, id: r.id })),
            });
            const rate = (label, records, predicate, explanation, key = "os") => component(label, records.length
                ? (100 * records.filter(predicate).length) / records.length
                : null, explanation, records, key);
            const self = (area) => {
                const records = rows(s, "lifeRating").filter((r) => r.area === area && recent(r));
                return component("Ressenti · " + area, mean(records.map((r) => Q.OS.n(r, "score") * 10)), "Moyenne des auto-évaluations de ce domaine sur les 30 derniers jours.", records);
            };
            const combine = (id, name, components) => {
                const available = components.filter((c) => c.score !== null);
                return {
                    id,
                    name,
                    score: available.length
                        ? Math.round(available.reduce((a, c) => a + c.score, 0) / available.length)
                        : null,
                    coverage: `${available.length}/${components.length} composante(s) renseignée(s)`,
                    components,
                };
            };
            const budgets = rows(s, "budget").filter((r) => r.month === today.slice(0, 7));
            const transactions = s.finances.filter((t) => !t.deleted &&
                String(t.date).startsWith(today.slice(0, 7)) &&
                String(t.date) <= today);
            const budgetValues = budgets.flatMap((b) => {
                const tx = transactions.filter((t) => Q.Personal.fold(t.category).trim() === Q.Personal.fold(b.category).trim());
                if (!tx.length)
                    return [];
                const spent = -tx
                    .filter((t) => Number(t.amount) < 0)
                    .reduce((a, t) => a + Number(t.amount), 0), limit = Q.OS.n(b, "limit");
                return [spent <= limit ? 100 : limit > 0 ? (100 * limit) / spent : 0];
            });
            const milestones = rows(s, "milestone");
            const courses = rows(s, "course");
            const programs = rows(s, "trainingProgram"), sessions = rows(s, "trainingSession").filter((r) => String(r.date) >= week &&
                String(r.date) <= today &&
            ...
            const docs = s.documents.filter((r) => Q.Personal.visible(r) && Q.validDate(String(r.expiry)));
            const chores = rows(s, "chore").filter((r) => String(r.due) >= since && String(r.due) <= today);
            const services = rows(s, "service").filter((r) => ["Activée", "Désactivée"].includes(String(r.mfa)));
            const impactGoals = rows(s, "impactGoal");
            const impact = impactGoals.flatMap((g) => {
                const contributions = rows(s, "contribution").filter((c) => c.impactGoalId === g.id && String(c.date) <= today);
                return contributions.length
                    ? [
                        Q.OS.clamp((100 * Q.OS.sum(contributions, "quantity")) /
                            Q.OS.n(g, "targetValue")),
                    ]
                    : [];
            });
            ...
            return [
                combine("finance", "Finances", [
                    component("Budgets", mean(budgetValues), `${budgetValues.length}/${budgets.length} budgets du mois avec des mouvements datés jusqu’au bilan. 100 si respecté ; sinon plafond/dépenses × 100. Les catégories sans mouvement restent inconnues.`, budgets),
                    self("Finances"),
                ]),
                combine("projects", "Projets", [
                    component("Jalons", milestones.length
                        ? (100 *
                            milestones
                                .filter(Q.OS.done)
                                .reduce((a, r) => a + Q.OS.n(r, "weight"), 0)) /
                            milestones.reduce((a, r) => a + Q.OS.n(r, "weight"), 0)
                        : null, "Part des jalons terminés, pondérée par leur poids ; état actuel des projets.", milestones),
                    self("Travail"),
                ]),
                combine("learning", "Apprentissage", [
                    component("Parcours", mean(courses.map((c) => Q.OS.n(c, "progress"))), "Moyenne des avancements saisis des parcours actifs ; état actuel.", courses),
                    self("Apprentissage"),
                ]),
                combine("health", "Activité physique", [
                    component("Régularité", programs.length && sessions.length
                        ? (100 *
                            sessions.filter((r) => programs.some((p) => p.id === r.programId)).length) /
                            programs.reduce((a, p) => a + Q.OS.n(p, "weeklySessions"), 0)
                        : null, "Séances des sept derniers jours / séances hebdomadaires prévues, plafonné à 100. Sans séance enregistrée, données insuffisantes. Ce n’est pas une mesure de santé.", sessions),
                    self("Santé"),
                ]),
                combine("relations", "Relations", [
                    rate("Cadence de contact", contacts, (r) => Q.OS.daysBetween(String(r.lastContact), today) <= Q.OS.n(r, "cadence"), "Part des contacts connus encore dans la cadence choisie ; dates futures ignorées. Les contacts sans date sont exclus."),
                    self("Relations"),
                ]),
                combine("documents", "Administration", [
                    rate("Validité des documents", docs, (r) => String(r.expiry) >= today, "Part des documents du coffre dont l’expiration renseignée n’est pas dépassée ; autres documents exclus.", "documents"),
                ]),
                combine("household", "Foyer", [
                    rate("Tâches du foyer", chores, Q.OS.done, "Part des tâches du foyer à échéance dans les 30 derniers jours marquées terminées. Les tâches récurrentes représentent leur occurrence courante."),
                    self("Foyer"),
                ]),
                combine("digital", "Vie numérique", [
                    rate("Double authentification", services, (r) => r.mfa === "Activée", "Part des services vérifiés avec double authentification activée ; À vérifier et Non disponible sont exclus."),
                ]),
                combine("impact", "Impact", [
                    component("Engagements", mean(impact), "Moyenne des contributions/cibles par engagement avec contributions datées jusqu’au bilan, plafonnée à 100. Chaque engagement conserve sa propre unité.", impactGoals),
                    self("Sens"),
                ]),
                combine("balance", "Équilibre", [
                    component("Énergie déclarée", mean(energy.map((r) => Q.OS.n(r, "energy") * 10)), "Moyenne de l’énergie déclarée sur les 30 derniers jours, multipliée par 10.", energy),
                    self("Loisirs"),
                ]),
            ];
        }
        Progression.scores = scores;
    })(Progression = Q.Progression || (Q.Progression = {}));
})(Q || (Q = {}));
